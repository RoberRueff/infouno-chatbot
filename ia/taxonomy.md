...
| `Infouno\SaaS\Persistence` | `src/Persistence/` | **Aislamiento fail-closed** [Bloque D]: TenantScopedRepository (base con `guardScope()`), MissingTenantScopeException, LeadRepository (inc 2), ConsentRepository (inc 3, dominio Consents — tablas consents/lead_consents/leads). Los repos tenant-scoped extienden esta base; el guard estático (NoRawSqlOutsidePersistenceTest) prohíbe SQL crudo fuera de la capa de persistencia (allowlist legacy en 3: solo Bots). `Opportunity\OpportunityRepository` también extiende la base (inc 4). |
