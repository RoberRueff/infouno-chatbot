...
> Aislamiento de tenant fail-closed (Bloque D, incrementos 1-3): capa `Persistence\TenantScopedRepository` (`guardScope()` lanza si el scope ≤ 0) + guard estático en CI (`NoRawSqlOutsidePersistenceTest`) que bloquea SQL crudo fuera de la capa de persistencia. Dominios Leads y Consents ya migrados (`LeadRepository`, `ConsentRepository`; Controllers/Dashboard/Service sin `$wpdb`); el `ConsentRepository` cubre las 3 tablas de consentimiento (`consents`, `lead_consents`, `leads`) con scope por bot_id o tenant_id según la query. Allowlist legacy reducida a 5 (Opportunities/Bots), pendiente de los incrementos 4-5.
