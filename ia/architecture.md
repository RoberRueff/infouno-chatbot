...
> Monetización — MercadoPago Suscripciones premium (v11): suscripción recurrente en ARS vía preapproval. **Enfoque A**: el webhook (`/billing/webhook`) verifica la firma `x-signature` (HMAC + anti-replay) y luego **trae el estado autoritativo de la API de MP** antes de reconciliar — nunca confía en el body. `SubscriptionService` aplica la máquina de estados (authorized/approved→premium activo, rejected→suspended, cancelled→free) de forma idempotente (por `mp_payment_id`) y ordenada (`last_event_ts`). SQL en `Persistence\SubscriptionRepository` (fail-closed). Credenciales env-first + precio en WP Admin (`BillingSettings`). Tablas `wp_infouno_subscriptions` + `wp_infouno_payment_events`.
