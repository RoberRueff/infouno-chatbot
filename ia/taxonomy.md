...
| `Infouno\SaaS\Persistence` | `src/Persistence/` | **Aislamiento fail-closed** [Bloque D]: TenantScopedRepository (base con `guardScope()`), MissingTenantScopeException. Los repos tenant-scoped extienden esta base; el guard estático (NoRawSqlOutsidePersistenceTest) prohíbe SQL crudo fuera de la capa de persistencia |
