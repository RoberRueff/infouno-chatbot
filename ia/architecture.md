...
> Aislamiento de tenant fail-closed (Bloque D, incrementos 1-4): capa `Persistence\TenantScopedRepository` (`guardScope()` lanza si el scope ≤ 0) + guard estático en CI (`NoRawSqlOutsidePersistenceTest`) que bloquea SQL crudo fuera de la capa de persistencia. Dominios Leads, Consents y Opportunities ya migrados: `LeadRepository`, `ConsentRepository` (3 tablas de consentimiento) y `OpportunityRepository` (ahora extiende la base, `guardScope` en sus 9 métodos + `getLeadSnapshotForTenant`/`listWithLeadDataForTenant`); sus Controllers/Dashboards quedaron sin `$wpdb`. Allowlist legacy reducida a 3 (solo Bots), pendiente del incremento 5 (allowlist vacía → guard total).
